...
#wind increase is eigenlijk totale capaciteit
#solar increase is eigenlijk totale capaciteit

#filter df on last 24*30 values
df = df.iloc[-24*30:]

time_list = list(range(1, len(df['Total [kW]'].tolist())))
df['Solar PU [kW]'] = df['Solar [kW]'] / df['Solar [kW]'].sum()
df['Wind PU [kW]'] = df['Wind [kW]'] / df['Wind [kW]'].sum()
...
model.production_wi_profile = Param(model.times, initialize=df['Wind PU [kW]'].iloc[1:].to_dict())
model.production_so_profile = Param(model.times, initialize=df['Solar PU [kW]'].iloc[1:].to_dict())
