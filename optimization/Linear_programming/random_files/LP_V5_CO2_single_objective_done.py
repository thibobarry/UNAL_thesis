...
def sensitivity(combination):
    ...
    model.old_co2_new_param = Var(model.times, within = NonNegativeReals) # TOEVOEGEN AAN verslag !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    model.old_cost_new_param = Var(model.times, within = NonNegativeReals) # TOEVOEGEN AAN verslag !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    def old_co2_new_param(model, t): #berekent de oude co2 met de nieuwe parameters
    ...
    model.constraint_old_co2_new_param = Constraint(model.times, rule=old_co2_new_param)

    def old_cost_new_param(model, t): #berekent de oude cost met de nieuwe parameters
    ...
    model.constraint_old_cost_new_param = Constraint(model.times, rule=old_cost_new_param)
    ...
    data = { 
            ...
            'co2_conventional [kg]': 0,
            'co2_regression [kg]': 0,
            'original_cost_new_param [$]': sum((model.old_cost_new_param[t]) for t in model.times),
            'original_co2_new_param [kg]': sum((model.old_co2_new_param[t]) for t in model.times),
            }
    new_row = pd.DataFrame(data, index=[0])
    df_sensitivity = pd.concat([df_sensitivity, new_row], ignore_index=True)
    df_sensitivity.tail(1).to_csv('sensitivity_data_rand_1000_2.csv', mode='a', header=False, index=False)
...
if len(combinations_full) >= 1000:
    # Use random.sample to extract 1000 random elements.
    combinations = random.sample(combinations_full, 1000)
    #delete combinations in combinations_full that are in combinations
    for combination in combinations:
        combinations_full.remove(combination)
    combinations_full = random.sample(combinations_full, 1000)
else:
